...
test_default_convention() {
  echo "---- Begin test test-default-convention ----"
  process_manifest "test-default-convention/umbrel-app.yml"
  assert_version "test-default-convention" "v0.9.0"
  echo "---- End test test-default-convention ----"
}

test_source_service_name() {
  echo "---- Begin test test-source-service-name ----"
  process_manifest "test-source-service-name/umbrel-app.yml" "web"
  assert_version "test-source-service-name" "v0.9.0"
  echo "---- End test test-source-service-name ----"
}
